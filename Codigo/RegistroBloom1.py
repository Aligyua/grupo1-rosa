...
class GestorDeUsuarios:
    def __init__(self):
        self.conn = sqlite3.connect('bloom_music.db')
        self.cursor = self.conn.cursor()
    def guardar_usuario(self, datos):
        try:
            fecha_nacimiento = datos['fecha_nacimiento']
            self.cursor.execute("INSERT INTO Usuario (nombre_Usuario, email, contraseña, Fecha_registro) VALUES (?, ?, ?, ?)",
                                (datos['nombre'], datos['email'], datos['contraseña'], fecha_nacimiento))
            self.conn.commit()
