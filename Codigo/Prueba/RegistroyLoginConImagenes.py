...
class Login:
    ...
    def entrar(self):
        ...
        print(f"Database result for user: {nombre}")
        ...
        if usuario:
            ...
            self.entry_passw.delete(0,tkinter.END)
            self.entry_usuario.delete(0,tkinter.END)
...
class GestorDeUsuarios:
    ...
    def iniciar_sesion(self,datos):
        try:
            self.cursor.execute("SELECT * FROM Usuario WHERE nombre_usuario = ? AND contraseña = ?",(datos['nombre'], datos['contraseña']))
...
class RegistroII:
    def __init__(self):
        ...
        self.label_contra2 = Label (self.frame_inferior,
                                    text="Repita Contraseña",
                                    font=("Arial",18),
                                    bg= fondo,
                                    fg= "black"
                                    )
        self.label_contra2.grid(row=1, column=0, padx=10, sticky="e") 

        self.entry_contra2 = Entry(self.frame_inferior,
                                   text="RepetidaContraseña",
                                   font=("Arial",18),
                                   bg=fondo,
                                   show="·",
                                   fg="black")
        
        self.entry_contra2.grid(row=1, column=1, columnspan=3, padx=5, sticky="w")
        ...
        self.texto1 = Label (self.frame_superior,
                             text= "1 caracter especial"+"8 caracteres minimo"+"alternar mayusculas y minusculas"+"debe contener 1 numero",
                             font=("Calista MT", 12),
                             fg="#a6a6a6",
                             bg=fondo,
    ...
    def contraiguales(self,contra1,contra2):
        iguales = False
        if contra1 == contra2:
            iguales = True
            return iguales
    ...
    def validarcontra(self,contra,contrarep):
        try:
            ...
            if len(contra) < 8:
                return messagebox.showwarning("Error: Contraseña Invalida","La contraseña debe tener al menos 8 CARACTERES")
        
            if not(self.mayusculascontra(contra)):
                return messagebox.showwarning("Error: Contraseña Invalida","La contraseña debe tener al menos 1 (un) caracter en mayuscula")

            if not(self.minusculascontra(contra)):
                return messagebox.showwarning("Error: Contraseña Invalida","La contraseña debe tener al menos 1 (un) caracter en minuscula")
            
            if not(self.caractespecialcontra(contra)):
                return messagebox.showwarning("Error: Contraseña Invalida","La contraseña debe tener al menos 1 (un) caracter especial")

                
            if not(self.numeroscontra(contra)):
                return messagebox.showwarning("Error: Contraseña Invalida","La contraseña debe tener al menos 1 (un) caracter numerico")


            if len(contra) == "":
                return messagebox.showwarning("Error", "Por favor, ingrese una contraseña.")
        
            soniguales = self.contraiguales(contra,contrarep)
            if not(soniguales):
                return messagebox.showwarning("Error", "Las contraseñas no coinciden.")
        except:
            messagebox.showinfo("Procesando","Algo fallo")
        return (contra)
    ...
    def siguienteRegistro(self):
        ...
        error = self.validarcontra(contra,contrarep)
        try:
            if error:
                #self.validarcontra(contra,contrarep)
                self.entry_contra.destroy(0,tkinter.END)
                self.entry_contra2.delete(0,tkinter.END)
        except:
            if error == contra:
                datos_usuario['contraseña'] = contra
                self.ventana.destroy()
...
# Grupo de botones de opción (Radiobuttons)
    ...
    def GuardarBDD(self):
        ...
        try:
            ...
            messagebox.showerror("Error", "No se pudo registrar el usuario.  Intenta de nuevo.exe")
        except Exception as e:
            messagebox.showerror("Error", f"Ocurrió un error: {e}")
        self.ventana.destroy()
        Login()
